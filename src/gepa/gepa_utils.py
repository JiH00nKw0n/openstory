...
class GEPAState:
    ...
    def __init__(
        ...
        valset_base_score = base_valset_eval_output.score
        base_valset_pareto_front = base_valset_eval_output.results
        ...
        self.prog_candidate_val_subscores = [base_valset_eval_output.results]
...
def write_eval_output_to_directory(eval_out, output_dir):
    # Handle case where eval_out might not be in the expected (score, predictions, subscores) format
    try:
        subscores = eval_out[2] if isinstance(eval_out, (tuple, list)) and len(eval_out) > 2 else []
        predictions = eval_out[1] if isinstance(eval_out, (tuple, list)) and len(eval_out) > 1 else []

        for task_idx, score in enumerate(subscores):
            ...
            with open(os.path.join(output_dir, f"task_{task_idx}", f"iter_{0}_prog_0.json"), 'w') as f:
                if task_idx < len(predictions):
                    json.dump(predictions[task_idx], f, indent=4, default=json_default)
                else:
                    json.dump({"score": score}, f, indent=4, default=json_default)
    except (IndexError, TypeError) as e:
        # If eval_out is not in expected format, create minimal output directory structure
        print(f"Warning: eval_out format unexpected, creating minimal structure: {e}")
        os.makedirs(output_dir, exist_ok=True)
        with open(os.path.join(output_dir, "eval_summary.json"), 'w') as f:
            json.dump({"eval_out": str(eval_out)}, f, indent=4, default=json_default)
...
def initialize_gepa_state(gepa_state_to_use, run_dir, logger, base_dspy_program, trainset_evaluator, valset_evaluator, seed, run_linearized_gepa, track_scores_on, train_val_size):
    if gepa_state_to_use is None:
        if os.path.exists(os.path.join(run_dir, "gepa_state.bin")) and os.path.exists(os.path.join(run_dir, "prog_candidates")):
            ...
            num_evals_run += len(valset_out.results)
