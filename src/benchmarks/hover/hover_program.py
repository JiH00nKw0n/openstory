...
def init_retriever():
    ...
    with init_lock:
        if not initialized:
            if not os.path.exists(os.path.join(os.path.dirname(__file__), "bm25s_retriever")) or not os.path.exists(
...
def provide_feedback_to_summary_module(
    ...
    if score:
        ...
        if "context" in predictor_inputs:
            ...
            docs_helped_retrieval_by_this_summary = remaining_docs_to_retrieve_after_hop2.difference(
                remaining_docs_to_retrieve_at_end
            )
            docs_remaining_after_hop2_not_helped = remaining_docs_to_retrieve_after_hop2.intersection(
                remaining_docs_to_retrieve_at_end
            )
...
Think about how you can make the connection between the provided passages and the missed evidence relevant to the claim."""
            ...
            docs_helped_retrieval_by_this_summary = remaining_docs_to_retrieve_after_hop1.difference(
                remaining_docs_to_retrieve_at_end
            )
            docs_remaining_after_hop1_not_helped = remaining_docs_to_retrieve_after_hop1.intersection(
                remaining_docs_to_retrieve_at_end
            )
...
def provide_feedback_to_query_module(
    ...
    if score:
        ...
        if "summary_2" in predictor_inputs:
            ...
            remaining_docs_to_retrieve_after_hop2 = gold_titles.difference(docs_after_hop2)
            remaining_docs_to_retrieve_at_end = gold_titles.difference(docs_after_hop3)
            docs_helped_retrieval_by_this_query = remaining_docs_to_retrieve_after_hop2.difference(
                remaining_docs_to_retrieve_at_end
            )
            docs_remaining_after_hop2_not_helped = remaining_docs_to_retrieve_after_hop2.intersection(
                remaining_docs_to_retrieve_at_end
...
Think about how you can modify your query to make the connection between the provided summary and the missed evidence relevant to the claim."""
            ...
            docs_helped_retrieval_by_this_query = remaining_docs_to_retrieve_after_hop1.difference(
                remaining_docs_to_retrieve_after_hop2
            )
            docs_remaining_after_hop1_not_helped = remaining_docs_to_retrieve_after_hop1.intersection(
                remaining_docs_to_retrieve_after_hop2
            )
