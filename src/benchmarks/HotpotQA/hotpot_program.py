...
def get_textual_context(d):
    title_to_sentences = {title: sentences for title, sentences in
                          zip(d['context']['title'], d['context']['sentences'])}
    text = ""
...
def provide_feedback_to_query_module(
    ...
    title_to_sentences = {title: sentences for title, sentences in
                          zip(module_inputs['context']['title'], module_inputs['context']['sentences'])}
...
def provide_feedback_to_summary2_module(
    ...
    ideal_summary_2 = "\n   ".join(
        [f"{title} | {title_to_sentences[title][sent_id]}" for title, sent_id in
         zip(module_inputs['supporting_facts']['title'], module_inputs['supporting_facts']['sent_id'])]
        )
